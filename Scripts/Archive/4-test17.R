...
saveRDS(fit, file = paste0(args, '-iar-stan_output-17-', IAR_out_date, '.rds'))

#save data to RDS (has which cells are modeled)
saveRDS(DATA, file = paste0(args, '-iar-stan_input-17-',  IAR_out_date, '.rds'))
...
sink(paste0(args, '-iar-stan_results-17-', IAR_out_date, '.txt'))
