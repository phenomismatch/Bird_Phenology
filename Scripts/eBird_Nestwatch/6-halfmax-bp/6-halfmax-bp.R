...
m_adults <- dplyr::filter(m_mf, age %in% c('1', '5', '6', '7', '8'))
...
ifelse(!dir.exists(paste0(dir, 'Bird_Phenology/Figures/halfmax/bp_', RUN_DATE)),
       dir.create(paste0(dir, 'Bird_Phenology/Figures/halfmax/bp_', RUN_DATE)),
       FALSE)

setwd(paste0(dir, 'Bird_Phenology/Figures/halfmax/bp_', RUN_DATE))
...
{
  #j <- 1
  ydata <- dplyr::filter(m_bp_f, year == years[j])
  ...
  for (k in 1:ncell)
  {
    ...
    if (n1 > 5 & n0 > 5 & njd0i > 3 & njd1 > 3 & njd > 9)
    {
      ...
      pdf(paste0(args, '_', years[j], '_', cells[k], '_bp.pdf'))
...
#save to rds object
setwd(paste0(dir, '/Bird_Phenology/Data/Processed/halfmax_bp_', RUN_DATE))
saveRDS(halfmax_df, file = paste0('halfmax_bp_', args, '.rds'))
