######################
# 7 - juv hitting nets ~ brood patch
#
######################
...
bp_master <- readRDS(paste0('bp-output-', juv_date, '.rds'))
...
#merge data sets
mrg <- dplyr::inner_join(juvs_master, bp_master, by = c('species', 'cell', 'year'))

#only species/cells/years with data for juvs and bp
mrg_f <- dplyr::filter(mrg, !is.na(juv_mean), !is.na(bp_mean))
...
setwd(paste0(dir, 'Bird_Phenology/Data/Processed/br_arr_', juv_date))
saveRDS(fit, file = paste0('juv-bp-stan-output-', juv_date, '.rds'))
...
sink(paste0('juv-bp-stan-results-', juv_date, '.txt'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   priors = PR,
                   pdf = TRUE,
                   filename = paste0('juv-bp-', juv_date, '-trace_mu_alpha.pdf'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   priors = PR,
                   pdf = TRUE,
                   filename = paste0('juv-bp-', juv_date, '-trace_mu_beta.pdf'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   priors = PR,
                   pdf = TRUE,
                   filename = paste0('juv-bp-', juv_date, '-trace_sigma_sp[1].pdf'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   priors = PR,
                   pdf = TRUE,
                   filename = paste0('juv-bp-', juv_date, '-trace_sigma_sp[2].pdf'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   priors = PR,
                   pdf = TRUE,
                   filename = paste0('juv-bp-', juv_date, '-trace_sigma.pdf'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   priors = PR,
                   pdf = TRUE,
                   filename = paste0('juv-bp-', juv_date, '-trace_mu_juv.pdf'))
...
data_vis_fun <- function(SPECIES = 'all')
{
  ...
  x_true_mean <- MCMCvis::MCMCpstr(fit, params = 'mu_x', type = 'summary', 
                                   func = mean)[[1]]
  x_true_LCI <- MCMCvis::MCMCpstr(fit, params = 'mu_x', type = 'summary', 
                                  func = function(x) quantile(x, probs = c(0.025)))[[1]]
  x_true_UCI <- MCMCvis::MCMCpstr(fit, params = 'mu_x', type = 'summary', 
                                  func = function(x) quantile(x, probs = c(0.975)))[[1]]
  ...
  DATA_PLOT <- data.frame(mean_y = y_true_mean,
                          ...
                          x_obs = DATA$x,
                          x_obs_l = DATA$x - (1.96 * DATA$sd_x),
                          x_obs_u = DATA$x + (1.96 * DATA$sd_x),
