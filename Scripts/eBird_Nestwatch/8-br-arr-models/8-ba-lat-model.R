######################
# 8 - broop patch - arrival ~ cell lat
#
######################
...
mrg_f3$ba <- mrg_f3$bp_mean - mrg_f3$mean_post_IAR
mrg_f3$sd_ba <- sqrt(mrg_f3$bp_sd^2 + mrg_f3$sd_post_IAR^2)



# Stan model --------------------------------------------------------------

DATA <- list(y = mrg_f3$ba,
             sd_y = mrg_f3$sd_ba,
             N = NROW(mrg_f3),
             sp = sp_idx,
...
setwd(paste0(dir, 'Bird_Phenology/Data/Processed/br_arr_', bp_date))
saveRDS(fit, file = paste0('ba-lat-stan-output-', bp_date, '.rds'))
#fit <- readRDS(paste0('ba-lat-stan-output-', bp_date, '.rds'))
...
sink(paste0('ba-lat-stan-results-', DATE, '.txt'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   ind = TRUE,
                   pdf = TRUE,
                   filename = paste0('ba-lat-', bp_date, '-trace_mu_alpha.pdf'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   ind = TRUE,
                   pdf = TRUE,
                   filename = paste0('ba-lat-', bp_date, '-trace_mu_beta.pdf'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   ind = TRUE,
                   pdf = TRUE,
                   filename = paste0('ba-lat-', bp_date, '-trace_sigma_sp[1].pdf'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   ind = TRUE,
                   pdf = TRUE,
                   filename = paste0('ba-lat-', bp_date, '-trace_sigma_sp[2].pdf'))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   ind = TRUE,
                   pdf = TRUE,
                   filename = paste0('ba-lat-', bp_date, '-trace_sigma.pdf'))
