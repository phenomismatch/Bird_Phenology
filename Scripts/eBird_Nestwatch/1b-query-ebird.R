...
for (i in 1:nsp)
{
  ...
  data2[ind, paste0(species_list_i[i,1])] <- 1
  
  #fill no observations for species i with 0s
  data2[n_ind, paste0(species_list_i[i,1])] <- 0
}
...
SJDAY  <- as.vector(scale(data3$day, scale = FALSE))
SJDAY2 <- as.vector(scale(data3$day^2, scale = FALSE))
SJDAY3 <- as.vector(scale(data3$day^3, scale = FALSE))
...
#scaled effort hours
SHR <- as.vector(scale((data3$duration_minutes/60)))
