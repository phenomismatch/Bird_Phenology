...
#Species, years, cells investigated
...
t_mat <- matrix(data = NA, nrow = 4, ncol = ((ITER/2)*CHAINS))
colnames(t_mat) <- paste0('iter_', 1:((ITER/2)*CHAINS))
halfmax_df <- data.frame(species = args, 
                         year = 2014, 
                         cell = cells, 
...
formulas <- list(f3 = 'detect ~ sjday + sjday2 + sjday3 + shr',
  ...
  for (k in 1:ncell)
  {
    ...
    #{
      for (m in 1:length(formulas))
      {
        ...
        nm <- paste0(args, '_', years[j], '_', cells[k], '-poly_', m + 2)
        assign(nm, tfit)
        ...
        pdf(paste0(args, '_', years[j], '_', cells[k], '-model_num_', m, '_arrival.pdf'))
        ...
        #halfax mean and sd
        ind_mean <- grep(paste0('hf_mean_m', m), colnames(halfmax_df))
        ind_sd <- grep(paste0('hf_sd_m', m), colnames(halfmax_df))
        
        halfmax_df[counter, ind_mean] <- mean(halfmax_fit)
        halfmax_df[counter, ind_sd] <- sd(halfmax_fit)
        
        
        #waic and se
        ind_waic <- grep(paste0('waic_', m), colnames(halfmax_df))
        ind_waic_se <- grep(paste0('waic_se_', m), colnames(halfmax_df))
        
        halfmax_df[counter, ind_waic] <- waic(tfit)$estimates[3, 1]
        halfmax_df[counter, ind_waic_se] <- waic(tfit)$estimates[3, 2]
      }
