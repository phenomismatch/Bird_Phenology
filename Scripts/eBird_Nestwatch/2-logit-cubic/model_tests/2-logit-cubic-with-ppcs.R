...
for(f in 1:length(formulas)){
  ppccs[[f]] <- list()
counter <- 1
hdflist[[f]] <- data.frame(species = args, 
...
for (j in 1:nyr)
{
  ...
  for (k in 1:ncell)
  {
    ...
    hdflist[[f]]$n1[counter] <- n1
    hdflist[[f]]$n1W[counter] <- n1W
    hdflist[[f]]$n0[counter] <- n0
    hdflist[[f]]$n0i[counter] <- n0i
    hdflist[[f]]$njd1[counter] <- njd1
    hdflist[[f]]$njd0[counter] <- njd0
    hdflist[[f]]$njd0i[counter] <- njd0i
    ...
    {
      fit2 <- rstanarm::stan_glm(formulas[[f]],
                                 data = cyspdata,
      ...
      hdflist[[f]]$num_diverge[counter] <- num_diverge
      hdflist[[f]]$num_tree[counter] <- num_tree
      hdflist[[f]]$num_BFMI[counter] <- num_BFMI
      hdflist[[f]]$delta[counter] <- DELTA
      hdflist[[f]]$tree_depth[counter] <- TREE_DEPTH
      
      #generate predict data
      pds <- range(cyspdata$day)[1]:range(cyspdata$day)[2]
      predictDays <- scale(pds)
      predictDays2 <- scale(pds^2)
      predictDays3 <- scale(pds^3)
      predictDays4 <- scale(pds^4)
      predictDays4 <- scale(pds^5)
      predictDays4 <- scale(pds^6)
      predictDays4 <- scale(pds^7)
      ...
      for (L in 1:((ITER/2)*CHAINS))
      {
        ...
        halfmax_fit[L] <- pds[min(which(rowL > (max(rowL)/2)))]
      }
      ...
      iter_ind <- grep('iter', colnames(hdflist[[f]]))
      hdflist[[f]][counter,iter_ind] <- halfmax_fit
      hdflist[[f]]$max_Rhat[counter] <- round(max(summary(fit2)[, "Rhat"]), 2)
      hdflist[[f]]$min_neff[counter] <- min(summary(fit2)[, "n_eff"])
    }
    counter <- counter + 1
  } #k
} #j
...
#save to rds object
setwd(paste0(dir, '/Bird_Phenology/Data/Processed/halfmax_species_', RUN_DATE))
saveRDS(hdflist, file = paste0('halfmax_df_list_arrival_', args, '.rds'))
saveRDS(ppcs, file = paste0('ppcs_posterior_predictive_checks', args, '.rds'))
