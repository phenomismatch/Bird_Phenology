######################
# X - halfmax model brood patch MAPS
#
# Fit model (GAM logistic regression) to MAPS data to get half-max parameter (brood patch) for each species-cell-year
#
# y ~ bernouli(p)
# logit(p) = s(day)
#
...
if (NROW(nrng@data) > 0)
{
  ...
  maps_data2 <- maps_data[which(maps_data$cell %in% cells),]
  ...
  #save to rds object
  setwd(paste0(dir, '/Bird_Phenology/Data/Processed/halfmax_brood_patch_', RUN_DATE))
  saveRDS(halfmax_df, file = paste0('halfmax_df_brood_patch_', args, '.rds'))
...
maps_adults <- dplyr::filter(maps_data2, age %in% c('1', '5', '6', '7', '8'))
...
years <- min(maps_bp_f$year):max(maps_bp_f$year)
nyr <- length(years)
...
ifelse(!dir.exists(paste0(dir, 'Bird_Phenology/Figures/halfmax/brood_patch_', RUN_DATE)), 
       dir.create(paste0(dir, 'Bird_Phenology/Figures/halfmax/brood_patch_', RUN_DATE)), 
       FALSE)

setwd(paste0(dir, 'Bird_Phenology/Figures/halfmax/brood_patch_', RUN_DATE))
...
{
  #j <- 13
  yspdata <- maps_bp_f[which(maps_bp_f$year == years[j]), ]
  ...
  for (k in 1:ncell)
  {
    ...
    n0 <- sum(cyspdata$bp == 0)
    #number of detections that came before jday 60
    n1W <- sum(cyspdata$bp * as.numeric(cyspdata$day < 60))
    #number of unique days with detections
    njd1 <- length(unique(cyspdata$day[which(cyspdata$bp == 1)]))
    #number of unique days with non-detection
    njd0 <- length(unique(cyspdata$day[which(cyspdata$bp == 0)]))
    
    
    if (n1 > 0)
    {
      #number of unique days of non-detections before first detection
      njd0i <- length(unique(cyspdata$day[which(cyspdata$bp == 0 & cyspdata$day < 
                                                  min(cyspdata$day[which(cyspdata$bp == 1)]))]))
      #number of non-detections before first detection
      n0i <- length(which(cyspdata$bp == 0 & 
                            cyspdata$day < min(cyspdata$day[which(cyspdata$bp == 1)])))
    } else {
      njd0i <- 0
      n0i <- 0
    }
    ...
    if (n1 > 5 & n0 > 15 & njd0i > 5 & njd1 > 5)
    {
      ...
      pdf(paste0(args, '_', years[j], '_', cells[k], '_brood_patch.pdf'))
