######################
# 4 - IAR model - spatial component on gamma (cell random effect)
#
# Fit IAR model
#
# VVV MODEL VVV
...
# \mu_{[i,j]} = \beta_{0[j]} + \gamma_{[i]}
# \beta_{0[j]} \sim N(0, \sigma_{\beta_{0}})
# \gamma_{[i]} \sim N(\mu_{\gamma[i]}, \sigma_{\gamma})
# \mu_{\gamma[i]} = \alpha_{\gamma} + \beta_{\gamma} * lat_{[i]} + \phi_{[i]} * \sigma_{\phi}
# \sigma_{\phi[j]} \sim N(0, 5)
# \sigma_{\beta_{0}} \ sim HN(0, 5)
# \alpha_{\gamma} \sim N(0, 30)
# \beta_{\gamma} \sim N(2, 3)
# \phi_{[i]} \sim N(0, [D - W]^{-1})
# \forall j \in \left \{1, ..., J  \right \}; \sum_{i} \phi_{[i]} = 0
######################
...
real y_obs[N, J];             // observed response data (add NAs to end)
...
mu_gamma = alpha_gamma + beta_gamma * lat + phi * sigma_phi;
...
fit <- rstan::stan(model_code = IAR_2,
                   ...
                   pars = c('sigma_beta0', 'beta0', 
                            'alpha_gamma', 'beta_gamma', 'sigma_gamma',
                            'sigma_phi', 'sigma_y_true', 'gamma', 'phi', 
                            'y_true', 'y_rep'),
...
# fit <- readRDS('Empidonax_virescens-2019-05-25-iar-stan_output-new.rds')
...
# bayesplot::ppc_dens_overlay(n_y_PPC, n_t_y_rep[1:100,])
