...
setwd(paste0(dir, 'Bird_Phenology/Data/Processed/arr_br_', bp_date))
...
mu_y_raw ~ std_normal();
mu_bp_raw ~ std_normal();

to_vector(z) ~ std_normal();
L_Rho ~ lkj_corr_cholesky(2);

// observation model for bp
bp ~ normal(mu_bp, sd_bp);
...
setwd(paste0(dir, 'Bird_Phenology/Data/Processed/arr_br_', bp_date))
saveRDS(fit, file = paste0('arr-br-bp-stan-output', bp_date, '.rds'))
...
sink(paste0('arr-br-bp-stan-results-', DATE, '.txt'))
...
data_vis_fun <- function(SPECIES = 'all')
{
  ...
  x_true_mean <- MCMCvis::MCMCpstr(fit, params = 'mu_bp', type = 'summary', 
                                   func = mean)[[1]]
  x_true_LCI <- MCMCvis::MCMCpstr(fit, params = 'mu_bp', type = 'summary', 
                                  func = function(x) quantile(x, probs = c(0.025)))[[1]]
  x_true_UCI <- MCMCvis::MCMCpstr(fit, params = 'mu_bp', type = 'summary', 
                                  func = function(x) quantile(x, probs = c(0.975)))[[1]]
  ...
  DATA_PLOT <- data.frame(mean_y = y_true_mean,
                          ...
                          x_obs = DATA$bp,
                          x_obs_l = DATA$bp - (1.96 * DATA$sd_bp),
                          x_obs_u = DATA$bp + (1.96 * DATA$sd_bp),
