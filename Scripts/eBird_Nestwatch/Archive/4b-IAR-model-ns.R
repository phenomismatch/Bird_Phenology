######################
# 3b - ICAR model with both spatial and non-spatial components
#
# Fit ICAR model - true arrival dates for each species-cell-year are modeled as latent states, with the observed 
# state derived from 2-logit-cubic.R. Same as 3-ICAR-model, except that a non-spatial component is used in addition
# to the spatial component.
#
# Formerly NA_birdPhen2.R
######################
...
grid <- dgcellstogrid(hexgrid6, as.integer(cells_frame$cell), frame=TRUE,wrapcells=TRUE)
...
diag_frame$spyr <- paste(diag_frame$species, diag_frame$year, sep="_")
spyrs <- unique(diag_frame$spyr)
nfits <- length(spyrs)
ncores <- 4
...
for(i in 1:ncores){
  ...
  for(j in 1:ceiling(nfits/ncores)){
    ...
    if(counter <= nfits){
      Ldatalist[[i]][[j]] <- 
        diag_frame[which(diag_frame$spyr == spyrs[counter]), ]
    }
...
years <- 2002:2016
frame_sp <- diag_frame[which(diag_frame$species == species_list[i]), ]
frame_sp_yr <- frame_sp[which(frame_sp$year == years[j]), ]
