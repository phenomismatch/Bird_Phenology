...
run_date <- '2020-12-29'
br_IAR_out_dir <- paste0('br_IAR_hm_', run_date)
...
br_na <- which(br_master$VALID == FALSE)
br_master$br_GAM_mean[br_na] <- NA
br_master$br_GAM_sd[br_na] <- NA


#filter by year and species
f1 <- dplyr::filter(br_master, year >= 2002, year <= 2017, per_ovr >= 0.05, 
                      species == args[1], breed_cell == TRUE, other_cell == FALSE)
tt3 <- dplyr::filter(f1, !is.na(br_GAM_mean))
if (NROW(tt3) < 3)
{
  sink(paste0(dir, 'Bird_Phenology/Data/Processed/', br_IAR_out_dir, '/', args[1], '-error.txt'), 
       type="output")
...
if (NROW(dplyr::filter(f1, !is.na(br_GAM_mean))) > 0)
{
  agg_br <- aggregate(br_GAM_mean ~ year, data = f1, function(x) sum(!is.na(x)))
} else {
  agg_br <- data.frame(year = NA, br_GAM_mean = NA)
}

#filter for valid years
vyrs <- dplyr::filter(agg_br, br_GAM_mean >= 3)$year

f2 <- dplyr::filter(f1, year %in% vyrs)

#ensure that cells are ordered
f3 <- dplyr::arrange(f2, species, cell, year)

#stop if species has fewer than 3 valid years
v_idx <- which(!is.na(f3$br_GAM_mean))
df <- f3[v_idx,]
...
cells <- sort(unique(f3$cell))
years <- sort(unique(f3$year))
nyr <- length(years)
ncell <- length(cells)
...
if (length(to.rm.ind) > 0)
{
  ...
  ncell <- length(cells)
  f3 <- dplyr::filter(f3, cell %in% cells)
  cellcenters <- dggridR::dgSEQNUM_to_GEO(hexgrid6, cells)
...
{
  #i <- 1
  temp_yr <- dplyr::filter(f3, year == years[i])
...
#save to RDS
saveRDS(fit, file = paste0(args[1], '-br-iar-hm-stan_output-', run_date, '.rds'))

#save data to RDS (has which cells are modeled)
saveRDS(DATA, file = paste0(args[1], '-br-iar-hm-stan_input-',  run_date, '.rds'))
...
sink(paste0(args[1], '-br-iar-hm-stan_results-', run_date, '.txt'))
cat(paste0('br IAR hm results ', args[1], ' \n'))
...
ifelse(!dir.exists(paste0(dir, 'Bird_Phenology/Figures/pre_post_IAR_maps/', br_IAR_out_dir)),
       dir.create(paste0(dir, 'Bird_Phenology/Figures/pre_post_IAR_maps/', br_IAR_out_dir)),
       FALSE)


setwd(paste0(dir, 'Bird_Phenology/Figures/pre_post_IAR_maps/', br_IAR_out_dir))
...
for (i in 1:length(years))
{
  ...
  f_out_filt <- dplyr::filter(f3, year == years[i])
