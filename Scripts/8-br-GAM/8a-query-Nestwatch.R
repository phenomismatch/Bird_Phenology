...
nestwatch_data <- DBI::dbGetQuery(cxn, paste0("SELECT lng, lat, year, day, common_name, sci_name,
                                              ...
                                              WHERE places.dataset_id = 'nestwatch'
...
nw_pro <- data.frame(species = rep(NA, length(uaid)), 
                     year = NA, 
                     lat = NA, 
                     lng = NA, 
                     attempt_id = NA, 
...
for (i in 1:length(uaid))
{
  ...
  nw_pro$lay[i] <- lubridate::yday(lubridate::parse_date_time(substr(temp$lay, 1, 9), orders = "dbY"))[1]
  nw_pro$hatch[i] <- lubridate::yday(lubridate::parse_date_time(substr(temp$hatch, 1, 9), orders = "dbY"))[1]
  nw_pro$fledge[i] <- lubridate::yday(lubridate::parse_date_time(substr(temp$fledge, 1, 9), orders = "dbY"))[1]
...
saveRDS(nw_pro, paste0('Nestwatch-pro-', RUN_DATE, '.rds'))
