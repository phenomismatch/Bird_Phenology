...
IAR_out_dir <- 'arrival_IAR_max_2020-06-08'
...
IAR_out_date <- substr(IAR_out_dir, start = 17, stop = 26)
...
f_out <- dplyr::filter(df_master, species == args[1] & MODEL_max == TRUE)

#fill invalid rows with NA
f_idx <- which(f_out$VALID_max == FALSE)
f_out$arr_GAM_max_mean[f_idx] <- NA
f_out$arr_GAM_max_sd[f_idx] <- NA
...
for (i in 1:nyr)
{
  ...
  sigma_y_in[i,] <- temp_yr$arr_GAM_max_sd
  
  for (j in 1:ncell)
  {
    #n <- 1
    #matrix with observed values with NAs
    y_PPC[counter] <- temp_yr$arr_GAM_max_mean[j]
    counter <- counter + 1
  }
  
  #which are not NA
  no_na <- temp_yr$arr_GAM_max_mean[which(!is.na(temp_yr$arr_GAM_max_mean))]
  ...
  if (length(no_na) < ncell)
  {
    ...
    t_obs_in <- c(which(!is.na(temp_yr$arr_GAM_max_mean)), rep(NA, num_na)) 
    t_mis_in <- c(which(is.na(temp_yr$arr_GAM_max_mean)), rep(NA, length(no_na)))
    ...
    y_obs_in[i,] <- no_na
    ii_mis_in[i,] <- which(!is.na(temp_yr$arr_GAM_max_mean))
    y_obs_in[i,] <- which(is.na(temp_yr$arr_GAM_max_mean))
  }
...
saveRDS(fit, file = paste0(args[1], '-iar-max-stan_output-', IAR_out_date, '.rds'))

#save data to RDS (has which cells are modeled)
saveRDS(DATA, file = paste0(args[1], '-iar-max-stan_input-',  IAR_out_date, '.rds'))
...
sink(paste0(args[1], '-iar-max-stan_results-', IAR_out_date, '.txt'))
cat(paste0('IAR max results ', args[1], ' \n'))
...
#f_rng <- c(range(f_out$arr_GAM_max_mean, na.rm = TRUE), range(med_fit, na.rm = TRUE))
#MIN <- round(min(f_rng))
#MAX <- round(max(f_rng))
...
for (i in 1:length(years))
{
  ...
  p <- ggplot() +
    ...
    geom_polygon(data = to_plt2, aes(x = long, y = lat.y, group = group, fill = arr_GAM_max_mean),
                 alpha = 0.4) +
    ...
    annotate('text', x = to_plt2$lon_deg, y = to_plt2$lat_deg + 0.5,
             label = round(to_plt2$arr_GAM_max_mean, digits = 0), col = 'black', alpha = 0.2,
             size = 4) +
    annotate('text', x = to_plt2$lon_deg, y = to_plt2$lat_deg - 0.5,
             label = round(to_plt2$arr_GAM_max_sd, digits = 0), col = 'white', alpha = 0.3,
             size = 3) +
