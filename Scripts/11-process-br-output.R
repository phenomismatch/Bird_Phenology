######################
# 11 - Extract breeding dates from breeding IAR output
#
...
IAR_out_dir <- 'br_IAR_hm_2020-12-29'
master_out_dir <- 'br_master_2020-12-29'
...
br_IAR_input <- readRDS(paste0('br_IAR_input_', br_date, '.rds'))
...
for (i in 1:length(species))
{
  ...
  if (length(grep(paste0(sp, '-br-iar-hm-stan_output-', IAR_out_date, '.rds'), list.files())) > 0)
  {
    
    #NA for species/year/cells with non-valid GAM results
    br_na <- which(br_IAR_input$VALID == FALSE)
    br_IAR_input$br_GAM_mean[br_na] <- NA
    br_IAR_input$br_GAM_sd[br_na] <- NA
    
    #filter by year and species
    f1 <- dplyr::filter(br_IAR_input, year >= 2002, year <= 2017, per_ovr >= 0.05, 
    ...
    t_fit <- readRDS(paste0(sp, '-br-iar-hm-stan_output-', IAR_out_date, '.rds'))
    t_data <- readRDS(paste0(sp, '-br-iar-hm-stan_input-', IAR_out_date, '.rds'))
    
    #only cells and years that were modeled (to account for any lone cells that were dropped in IAR)
    f_in <- dplyr::filter(f3, cell %in% t_data$cells)
    ...
    model_summary <- MCMCvis::MCMCsummary(t_fit, excl = c('br_rep'), round = 3)
    ...
    con <- file(paste0(sp, '-br-iar-hm-stan_results-', IAR_out_date, '.txt'), 'r')
    ...
    for (j in 1:length(t_years))
    {
      ...
      v_br_na <- which(is.na(t_f_in$VALID))
      if (length(v_br_na) > 0)
      {
        t_f_in$VALID[v_br_na] <- FALSE
      }
...
saveRDS(out2, file = paste0('br_master_', IAR_out_date, '.rds'))
